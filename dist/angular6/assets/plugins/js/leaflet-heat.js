...
L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _redraw: function () {
    ...
    // console.log(data);
    const avgIntensity = data.map(x => x[2]).reduce((a, c) => a + c, 0) / data.length;

    const newData = data.map(x => { x[2] = avgIntensity; return x });

    // console.log(newData);
    // console.log(newData[0][2]);
    // dynamicGrad = newData[0] - 1;
    // console.log(dynamicGrad);

    // this._heat.data(newData).gradient();
    // console.timeEnd('process');


    // console.time('draw ' + data.length);
    this._heat.data(newData).draw(this.options.minOpacity);
